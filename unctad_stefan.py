...
Original file is located at
    https://colab.research.google.com/drive/1hNQkuRcDOJE0nh64PxvcCOdWVTCgk9H7
"""
...
CSV_PATH = "/content/drive/Shareddrives/2025 Fall/Fall 2025 Projects/ECB/Week 4/dataset_goods.csv"
...
# ---------------------------
# Hold out final 'TEST_SIZE' points for OOS test
TEST_SIZE = 12  # months; adjust as needed
N = len(df)
...
for (n_steps, hidden, layers, drop, lr, bs, epochs) in grid_combos:
    ...
    splits = make_time_splits(N_trainval, n_splits=CV_SPLITS, fold_size=FOLD_SIZE, min_train_size=max(MIN_TRAIN, n_steps + 12))
    if len(splits) == 0:
...
# Test sequences:
# Build over the joined scaled array, then pick only those whose target time lies in the test region.
X_seq_all, y_seq_all = build_sequences(X_concat_scaled, y_all, n_timesteps=n_steps)
# Sequence index s corresponds to raw time index t = n_steps + s
# We want those with t in [N_trainval, N-1]
...
# test_res.to_csv("oos_predictions.csv", index=False)
# torch.save(model_best.state_dict(), "best_lstm_model.pt")
